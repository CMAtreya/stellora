...
const memories: MemoryCard[] = [
  {
    ...
    description:
      'The way the lights reflected off the asphalt felt like a movie scene. Met a local street artist who drew my TravelDNA on a napkin.',
    image: 'https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?auto=format&fit=crop&w=900&q=80',
  },
  {
    title: 'Mist Trail Climb',
    date: 'Aug 05, 2023',
    mood: 'Sour Memory',
    moodColor: 'text-rose-300',
    description:
      'Knee gave out halfway up. Beautiful view, but the descent was brutal. Note to self: always bring the titanium compression sleeves.',
    image: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=900&q=80',
  },
  {
    title: 'Kyoto Tea Ritual',
    date: 'Nov 20, 2023',
    mood: 'Sweet Memory',
    moodColor: 'text-cyan-300',
    description:
      'Total stillness. The TravelDNA monitor recorded my lowest resting heart rate ever. A profound sense of alignment.',
    image: 'https://images.unsplash.com/photo-1531190705574-f8f6b5ca55a3?auto=format&fit=crop&w=900&q=80',
  },
]
...
export default function ProfilePage() {
  const [walkingTolerance, setWalkingTolerance] = useState(15)
  const [memoryView, setMemoryView] = useState<'grid' | 'list'>('grid')
        ...
        <section className="mb-16">
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-[#c3c6d7]">Preserved moments from your journey</p>
            </div>
            <div className="flex gap-2">
              <button
                className={`rounded-lg p-2 transition ${memoryView === 'grid' ? 'bg-[#1f1f23] text-[#b4c5ff]' : 'bg-[#1f1f23] text-[#8d90a0]'}`}
                type="button"
                aria-label="Grid view"
                onClick={() => setMemoryView('grid')}
              >
                <Grid3X3 size={16} />
              </button>
            </div>
          ...
          <div className={memoryView === 'grid' ? 'grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3' : 'grid grid-cols-1 gap-5'}>
            {memories.map((memory) => (
              <div
                key={memory.title}
                className={
                  memoryView === 'grid'
                    ? 'group overflow-hidden rounded-3xl bg-[#1b1b1f] transition-colors duration-300 hover:bg-[#2a292e]'
                    : 'flex flex-col gap-4 rounded-3xl bg-[#1b1b1f] p-4 transition-colors duration-300 hover:bg-[#2a292e] md:flex-row'
                }
              >
                <div className={memoryView === 'list' ? 'relative h-56 w-full overflow-hidden rounded-2xl md:h-auto md:w-56 md:shrink-0' : 'relative h-64'}>
                  <img alt={memory.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" src={memory.image} />
                  <div className="absolute left-4 top-4">
                    <span className={`rounded-full bg-black/40 px-3 py-1 text-[10px] font-bold uppercase tracking-widest ${memory.moodColor}`}>
                      {memory.mood}
                    </span>
                  </div>
                </div>
                <div className={memoryView === 'list' ? 'flex flex-1 flex-col justify-between px-1 py-1' : 'p-6'}>
                  <div className="mb-4 flex items-start justify-between gap-4">
                    <h4 className="text-xl font-bold text-white">{memory.title}</h4>
                    <p className="text-xs font-medium uppercase text-[#8d90a0]">{memory.date}</p>
                  </div>
                  <p className="mb-6 text-sm leading-relaxed text-[#c3c6d7]">{memory.description}</p>
                  <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-tight text-[#b4c5ff]">
                    <NotebookPen size={14} />
                    View full note
                  </div>
                </div>
